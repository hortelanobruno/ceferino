...
//		f.show(); 
	...
	private void updateResult() {
		...
		// Check if expression was parsed without errors
		if (myParser.hasError()) {
			// The expression has an error
			resultLabel.setText("The expression could not be parsed");
			// Set the error information
			errorTextArea.setText(myParser.getErrorInfo());
		} else {
			// Try evaluating
			result = myParser.getValueAsObject();

			if (myParser.hasError()) {
				// something didn't work with the evaluation
				resultLabel.setText("The expression could not be evaluated");
				// set the error info
				errorTextArea.setText(myParser.getErrorInfo());
			} else {
				// Evaluation worked
